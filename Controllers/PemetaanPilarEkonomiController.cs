...
{
    public class PemetaanPilarEkonomiController : Controller
    {
        private readonly ApplicationDbContext _db;
        private readonly PilarService _pilarService;
        public PemetaanPilarEkonomiController(ApplicationDbContext db, PilarService pilarService)
        {
            _db = db;
        ...
        [HttpGet]
        public async Task<IActionResult> PilarEkonomi(int tahun_dokumen, string kode_cabang)
        {
            ...
            var tahunList = _db.TR_Ekonomi
                .Select(x => x.tahun_dokumen)
                .Distinct()
                .OrderByDescending(x => x)
                .ToList();

            var ctrData = await _db.CTR_Ekonomi
            ...
            var vwData = await _db.VW_KriteriaEkonomi
            ...
            ViewBag.EkonomiData = combinedData;
        ...
        [HttpGet]
        public async Task<IActionResult> DetailPilarEkonomi(string kriteria, string kode_cabang, int tahun_dokumen)
        {
            ...
            for(int i = 0; i < indikator.Count; i++){
                var transaksiEkonomi = await _db.TR_Ekonomi.Where(s => s.no_indikator == indikator[i].no_indikator 
                && s.kode_cabang == kode_cabang && s.tahun_dokumen == tahun_dokumen)
                .AsNoTracking().ToListAsync();
                if(transaksiEkonomi.Count != 0){
                    indikator[i].DTO_isian = transaksiEkonomi.FirstOrDefault().isian;
                    indikator[i].DTO_tahun_data = transaksiEkonomi.FirstOrDefault().tahun_data;
                    indikator[i].DTO_sumber_data = transaksiEkonomi.FirstOrDefault().sumber_data;
                }
            }
        ...
        [HttpPost]
        public IActionResult DetailPilarEkonomi(int id, List<VW_M_IndikatorAllPilar> Indikator, int tahun_dokumen, string kode_cabang)
        {
            try
            {
                ...
                if (!ModelState.IsValid)
                {
                    for(var i = 0; i < Indikator.Count(); i++){
                        var transaksiExist = _db.TR_Ekonomi
                        ...
                        if (id == 0 && transaksiExist == null) {
                            if(Indikator[i].DTO_isian !=null){
                                ...
                                _db.TR_Ekonomi.Add(Pilar);
                                _db.SaveChanges();
                            }
                        } else {
                            var existingPilar = _db.TR_Ekonomi.FirstOrDefault(
                            ...
                            if (existingPilar != null)
                            {
                                ...
                                _db.TR_Ekonomi.Update(existingPilar);
                                _db.SaveChanges();
                    ...
                    return RedirectToAction(nameof(PilarEkonomi), new { tahun_dokumen, kode_cabang });
                }
            }
            ...
            return RedirectToAction(nameof(PilarEkonomi), new { tahun_dokumen });
        }
