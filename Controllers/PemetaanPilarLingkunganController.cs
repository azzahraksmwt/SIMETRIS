...
{
    public class PemetaanPilarLingkunganController : Controller
    {
        private readonly ApplicationDbContext _db;
        private readonly PilarService _pilarService;
        public PemetaanPilarLingkunganController(ApplicationDbContext db, PilarService pilarService)
        {
            _db = db;
        ...
        [HttpGet]
        public async Task<IActionResult> PilarLingkungan(int tahun_dokumen, string kode_cabang)
        {
            ...
            var tahunList = _db.TR_Lingkungan
                .Select(x => x.tahun_dokumen)
                .Distinct()
                .OrderByDescending(x => x)
                .ToList();

            var ctrData = await _db.CTR_Lingkungan
            ...
            var vwData = await _db.VW_KriteriaLingkungan
            ...
            ViewBag.LingkunganData = combinedData;
        ...
        [HttpGet]
        public async Task<IActionResult> DetailPilarLingkungan(string kriteria, string kode_cabang, int tahun_dokumen)
        {
            ...
            for(int i = 0; i < indikator.Count; i++){
                var transaksiLingkungan = await _db.TR_Lingkungan.Where(s => s.no_indikator == indikator[i].no_indikator 
                && s.kode_cabang == kode_cabang && s.tahun_dokumen == tahun_dokumen).AsNoTracking().ToListAsync();
                if(transaksiLingkungan.Count != 0){
                    indikator[i].DTO_isian = transaksiLingkungan.FirstOrDefault().isian;
                    indikator[i].DTO_tahun_data = transaksiLingkungan.FirstOrDefault().tahun_data;
                    indikator[i].DTO_sumber_data = transaksiLingkungan.FirstOrDefault().sumber_data;
                }
            }
        ...
        [HttpPost]
        public IActionResult DetailPilarLingkungan(int id, List<VW_M_IndikatorAllPilar> Indikator, int tahun_dokumen, string kode_cabang)
        {
            try
            {
                ...
                if (!ModelState.IsValid)
                {
                    for(var i = 0; i < Indikator.Count(); i++){
                        var transaksiExist = _db.TR_Lingkungan
                        ...
                        if (id == 0 && transaksiExist == null) {
                            if(Indikator[i].DTO_isian !=null){
                                ...
                                _db.TR_Lingkungan.Add(Pilar);
                                _db.SaveChanges();
                            }
                        } else {
                            var existingPilar = _db.TR_Lingkungan.FirstOrDefault(
                            ...
                            if (existingPilar != null)
                            {
                                ...
                                _db.TR_Lingkungan.Update(existingPilar);
                                _db.SaveChanges();
                    ...
                    return RedirectToAction(nameof(PilarLingkungan), new { tahun_dokumen, kode_cabang });
                }
            }
            ...
            return RedirectToAction(nameof(PilarLingkungan), new { tahun_dokumen });
        }
