...
{
    public class PemetaanPilarKesehatanController : Controller
    {
        private readonly ApplicationDbContext _db;
        private readonly PilarService _pilarService;
        public PemetaanPilarKesehatanController(ApplicationDbContext db, PilarService pilarService)
        {
            _db = db;
        ...
        [HttpGet]
        public async Task<IActionResult> PilarKesehatan(int tahun_dokumen, string kode_cabang)
        {
            ...
            var tahunList = _db.TR_Kesehatan
                .Select(x => x.tahun_dokumen)
                .Distinct()
                .OrderByDescending(x => x)
                .ToList();

            var ctrData = await _db.CTR_Kesehatan
            ...
            var vwData = await _db.VW_KriteriaKesehatan
            ...
            ViewBag.KesehatanData = combinedData;
        ...
        [HttpGet]
        public async Task<IActionResult> DetailPilarKesehatan(string kriteria, string kode_cabang, int tahun_dokumen)
        {
            ...
            for(int i = 0; i < indikator.Count; i++){
                var transaksiKesehatan = await _db.TR_Kesehatan.Where(s => s.no_indikator == indikator[i].no_indikator 
                && s.kode_cabang == kode_cabang && s.tahun_dokumen == tahun_dokumen).AsNoTracking().ToListAsync();
                if(transaksiKesehatan.Count != 0){
                    indikator[i].DTO_isian = transaksiKesehatan.FirstOrDefault().isian;
                    indikator[i].DTO_tahun_data = transaksiKesehatan.FirstOrDefault().tahun_data;
                    indikator[i].DTO_sumber_data = transaksiKesehatan.FirstOrDefault().sumber_data;
                }
            }
        ...
        [HttpPost]
        public IActionResult DetailPilarKesehatan(int id, List<VW_M_IndikatorAllPilar> Indikator, int tahun_dokumen, string kode_cabang)
        {
            try
            {
                ...
                if (!ModelState.IsValid)
                {
                    for(var i = 0; i < Indikator.Count(); i++){
                        var transaksiExist = _db.TR_Kesehatan
                        ...
                        if (id == 0 && transaksiExist == null) {
                            if(Indikator[i].DTO_isian !=null){
                                ...
                                _db.TR_Kesehatan.Add(Pilar);
                                _db.SaveChanges();
                            }
                        } else {
                            var existingPilar = _db.TR_Kesehatan.FirstOrDefault(
                            ...
                            if (existingPilar != null)
                            {
                                ...
                                _db.TR_Kesehatan.Update(existingPilar);
                                _db.SaveChanges();
                    ...
                    return RedirectToAction(nameof(PilarKesehatan), new { tahun_dokumen, kode_cabang });
                }
            }
            ...
            return RedirectToAction(nameof(PilarKesehatan), new { tahun_dokumen });
        }
